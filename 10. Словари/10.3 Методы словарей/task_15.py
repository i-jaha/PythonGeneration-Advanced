...
Напишите программу, которая выводит слово, 
которое встречается реже всего, без учета регистра. 
...
text = [x.strip('.,!?:;-').lower() for x in input().split()]
result = {}
for word in text:
    result[word] = result.get(word, 0) + 1
min_count = min(result.values())
rare_words = [word for word, count in result.items() if count == min_count]
